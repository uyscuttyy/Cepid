...
export DEPLOYER_KEY="$DEMO_AGENT_PRIVATE_KEY"
export DEPLOY_ASSET=ETH
export DEPLOY_TIMEFRAME="${DEMO_MARKET_TIMEFRAME:-15M}"
export DEPLOY_DURATION="$DURATION"
export DEPLOY_MIN_SHARES="$MIN_SHARES"
export DEPLOY_RESOLVER="$RESOLVER"
export DEPLOY_RESERVE="${DEMO_MARKET_RESERVE:-20000000}"   # 20 USDC/side: retail trades move the price
...
# Wait for the approve receipt first: back-to-back sends race the same
# nonce and the second fails with "replacement transaction underpriced".
echo "[fund] approving USDC to market…"
APPROVE_TX=$(cast send "$USDC" "approve(address,uint256)" "$MARKET" "$FUND_AMOUNT" \
  --rpc-url "$RPC" --private-key "$DEMO_AGENT_PRIVATE_KEY" --json 2>/dev/null | python3 -c 'import json,sys; print(json.load(sys.stdin).get("transactionHash",""))')
echo "[fund] approve tx: $APPROVE_TX"
if [ -n "$APPROVE_TX" ]; then
  for _ in $(seq 1 30); do
    if cast receipt "$APPROVE_TX" --rpc-url "$RPC" 2>/dev/null | grep -q 'status.*1'; then break; fi
    sleep 4
  done
fi
